...
from src.classes import BaseProduct, Category, LawnGrass, Product, Smartphone, load_categories_from_json
...
def test_base_product_is_abstract() -> None:
    """BaseProduct нельзя инстанцировать."""
    with pytest.raises(TypeError):
        BaseProduct()  # type: ignore[abstract]


def test_product_calculate_total() -> None:
    """Проверка calculate_total у Product."""
    product = Product("Test", "desc", 100.0, 5)
    assert product.calculate_total() == 500.0


def test_mixin_prints_creation_info(capsys: CaptureFixture[str]) -> None:
    """Миксин печатает информацию о создании объекта Product."""
    Product("Тест", "Описание", 1200.0, 10)
    captured = capsys.readouterr()
